(function (e) {
    ...
    t.defaults = {axis: "xy", duration: parseFloat(e.fn.jquery) >= 1.3 ? 0 : 1, limit: true};
    t.window = function (t) {
    ...
    e.fn.scrollTo = function (r, i, s) {
        ...
        if (typeof s == "function")s = {onAfter: s};
        if (r == "max")r = 9e9;
        ...
        return this._scrollable().each(function () {
            ...
            e.each(s.axis.split(""), function (e, n) {
                ...
                if (f) {
                    ...
                    l[p] += s.offset[i] || 0;
                    if (s.over[i])l[p] += a[n == "x" ? "width" : "height"]() * s.over[i]
                } else {
                    var m = a[i];
                    l[p] = m.slice && m.slice(-1) == "%" ? parseFloat(m) / 100 * v : m
                }
                if (s.limit && /^\d+$/.test(l[p]))l[p] = l[p] <= 0 ? 0 : Math.min(l[p], v);
                if (!e && s.queue) {
                    if (d != l[p])h(s.onAfterFirst);
                    delete l[p]
                }
            });
            h(s.onAfter);
